...
#if defined(NDEBUG)
# error "Mirpay cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x71eed3d1829b1540b0039b1153c6b6075aaacd1c8970215a3f3da468219769fb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mirpay: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Mirpay Signed Message:\n";
...
{
    // Mirpay: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nPmCoin = 10000000000 * COIN;
static const int64 nSubsidy = 0 * COIN;
static const int64 nTargetTimespan = 24 * 60 * 60;  // retarget 1 days
static const int64 nTargetSpacing = 10; // 10s
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mirpay: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mirpay: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1bc38fde068c8336bd10bbb1650a4e29d315158a474e1085c1c933c65344ee7e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Taxi King - MirPay";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 200000000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048731e384e61cf15b587d5e247a57b8f1e7430f4e16e2a9d580c6266d38c66c00f2a022da49cfb9a68d5afc786969d1eb7ffc8017d31931f1151b3e2518cd3c19") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1531472317;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085989614;

        if (fTestNet)
        {
            block.nTime    = 1531472317;
            block.nNonce   = 385294371;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xee485f9b1b1572aec22ae9bf418437a2b3e4db2a311d21172e8d96b7ce08831c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xc5, 0xba, 0xd4, 0x2a }; // Mirpay: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MirpayMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MirpayMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MirpayMiner(CWallet *pwallet)
{
    printf("MirpayMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mirpay-miner");
    ...
    try { loop {
        ...
        printf("Running MirpayMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MirpayMiner, pwallet));
}
